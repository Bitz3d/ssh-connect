...
fn main() {
    ...
    let configuration = AppConfig::new().unwrap();

    let configuration: &ServerData = configuration.config(server_env);
    ...
    let download_path: String = configuration.download().path().to_owned() + filename;
